...
# Ev (kalkis/donus) konumu — planlama ekraninda tasinabilir, [0] ile sifirlanir
HOME_X_DEFAULT    = 0.0
HOME_Y_DEFAULT    = 0.0

# Waypoint rota sekli icin ornekleme yogunlugu (curve modunda, iki nokta arasi)
CURVE_SAMPLES_PER_SEGMENT = 10
...
WP_ACTIONS = ["none", "hover", "photo", "scan", "descend", "speed_slow", "speed_fast", "log"]
WP_ACTION_LABELS = {"none": "Gec", "hover": "Bekle", "photo": "Foto", "scan": "Tara",
                     "descend": "Alcal", "speed_slow": "YavasHiz", "speed_fast": "HizliHiz",
                     "log": "Isaret"}
WP_ACTION_COLORS = {"none": (150,150,170), "hover": (230,200,60),
                     "photo": (60,210,120), "scan": (60,220,200),
                     "descend": (230,140,40), "speed_slow": (100,140,230),
                     "speed_fast": (230,60,60), "log": (170,80,230)}
...
def _catmull_rom_point(p0, p1, p2, p3, t):
    """Standart uniform Catmull-Rom spline noktasi. t=0 -> p1, t=1 -> p2."""
    t2 = t * t
    t3 = t2 * t
...
def build_curve_path(points, samples=CURVE_SAMPLES_PER_SEGMENT):
    """points: [(x,y), ...] Orijinal noktalardan tam gecen yumusatilmis
    (Catmull-Rom) ara nokta listesi uretir. <3 nokta varsa duz cizgi doner."""
    n = len(points)
    if n < 3:
        return list(points)
...
class MissionRunner:
    def __init__(self, log_fn, frame_push_fn, settings_ref):
        ...
        self.wp_idx        = 0
        self.path_mode     = "line"    # "line" | "curve" (waypoint rotasi icin)
        self.home_x        = HOME_X_DEFAULT
        self.home_y        = HOME_Y_DEFAULT
        self.fly_speed     = SPIRAL_FLY_SPEED   # speed_slow/speed_fast waypoint action'larinca degistirilir
        self.scanning      = False   # scan waypoint'inden sonra baska (scan-disi) waypoint'e kadar surekli acik kalir

    def start(self, mode="normal", color_order="red_first",
             mission_type="spiral", waypoints=None,
             path_mode="line", home_x=HOME_X_DEFAULT, home_y=HOME_Y_DEFAULT):
        if self.running:
        ...
        self.home_x        = home_x
        self.home_y        = home_y
        self.fly_speed     = SPIRAL_FLY_SPEED
        self.scanning      = False
    ...
    # ── smooth yatay uçuş ───────────────────────────────────────
    # self.scanning acikken (scan waypoint'inden sonra, baska bir waypoint
    # bunu kapatana kadar) ucus boyunca surekli kamera goruntusu kontrol
    # edilir; renk gorulurse durup teslim edilir, sonra ucusa devam eder.
    def _fly_to(self, tx, ty, height=DRONE_HEIGHT):
        ...
        while not self.stop_evt.is_set():
            ...
            if dist < self.fly_speed:
                ...
                if self.scanning:
                    self._scan_check_and_deliver(tx, ty, log_no_detect=False)
                return True
            sx = (ddx/dist) * self.fly_speed
            sy = (ddy/dist) * self.fly_speed
            cx += sx; cy += sy
            move_to(cx, cy, height)
            self.drone_pos = {"x": cx, "y": cy, "z": height}
            time.sleep(SPIRAL_FLY_DELAY)
            if self.scanning and step % SPIRAL_SCAN_EVERY == 0:
                self._scan_check_and_deliver(cx, cy, log_no_detect=False)
            step += 1
        return False
    ...
    # ── su anki konumda kamera goruntusunu kontrol et, renk varsa git-teslim et-don ──
    # Hem waypoint'te dururken (scan/photo) hem de _fly_to icinde ucarken
    # (self.scanning acikken) kullanilir. Donen deger: bulundu mu (bool).
    def _scan_check_and_deliver(self, cur_x, cur_y, log_no_detect=True):
        bgr = get_image()
        if bgr is None:
            self.log("  UYARI: goruntu alinamadi (Blender'dan yanit yok).")
            return False
        self.push_frame(bgr)
        dets, _, _ = detect_colors(bgr)
        remaining = [c for c in ("red", "blue") if self.payloads.get(c)]
        found = [c for c in remaining if c in dets]
        if not found:
            if log_no_detect:
                # Teshis: renk maskesinde hic piksel var mi, yoksa MIN_AREA esiginin
                # altinda mi kaldi? Bir sonraki calistirmada nedenini gormek icin.
                hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
                mr = cv2.bitwise_or(cv2.inRange(hsv, RED_L1, RED_U1), cv2.inRange(hsv, RED_L2, RED_U2))
                mb = cv2.inRange(hsv, BLUE_L, BLUE_U)
                rpx, bpx = int(cv2.countNonZero(mr)), int(cv2.countNonZero(mb))
                self.log(f"  Tarama: renk yok (kirmizi_px={rpx}, mavi_px={bpx}, esik_alan={MIN_AREA})")
            return False
        resume_x, resume_y = cur_x, cur_y
        cache = {c: (cur_x, cur_y) for c in found}
        order = self._decide_order(found, cache)
        for color in order:
            if self.stop_evt.is_set():
                break
            if not self.payloads.get(color):
                continue
            label = "Kirmizi" if color == "red" else "Mavi"
            self.log(f"  {label} goruldu, gorevi icin gidiliyor...")
            ok = self._deliver(color, label, resume_x, resume_y)
            if not ok:
                self.log(f"  {label} icin gorev tamamlanamadi.")
        return True

    # Geriye donuk uyumluluk: waypoint nesnesi alan eski isim
    def _wp_scan_and_deliver(self, wp):
        return self._scan_check_and_deliver(wp.x, wp.y)
    ...
    # ─────────────────────────────────────────────────────────────
    def _fly_home(self):
        self.log(f"Eve donuluyor... ({self.home_x:.1f},{self.home_y:.1f})")
        self._fly_to(self.home_x, self.home_y)
    ...
    def _waypoint_mission(self):
        ...
        use_curve = (self.path_mode == "curve" and total >= 3)
        self.log(f"Rota basliyor: {total} konum ({'Egri' if use_curve else 'Duz'})")
        for i, wp in enumerate(self.waypoints):
            if self.stop_evt.is_set():
                break
            self.wp_idx = i
            self.log(f"  -> Konum {i+1}/{total} ({wp.x:.1f},{wp.y:.1f}) [{wp.action}]")
            if use_curve and i > 0:
                # Onceki/sonraki komsu noktalarla Catmull-Rom kontrol noktalari
                prev = self.waypoints[i-1]
                p0 = self.waypoints[i-2] if i-2 >= 0 else prev
                p3 = self.waypoints[i+1] if i+1 < total else wp
                pts4 = ((p0.x,p0.y), (prev.x,prev.y), (wp.x,wp.y), (p3.x,p3.y))
                for s in range(1, CURVE_SAMPLES_PER_SEGMENT + 1):
                    if self.stop_evt.is_set():
                        break
                    t = s / CURVE_SAMPLES_PER_SEGMENT
                    sx, sy = _catmull_rom_point(*pts4, t)
                    self._fly_to(sx, sy, wp.z)
            else:
                self._fly_to(wp.x, wp.y, wp.z)
            # Tarama modu: 'scan' aksiyonundan itibaren, scan-disi bir
            # waypoint'e gelene kadar sonraki tum ucus boyunca (_fly_to
            # icinde) surekli kamera goruntusu kontrol edilir.
            if wp.action == "scan":
                if not self.scanning:
                    self.log("  Tarama modu ACIK (bir sonraki scan-disi konuma kadar surekli)")
                self.scanning = True
                self._scan_check_and_deliver(wp.x, wp.y, log_no_detect=False)
            else:
                if self.scanning:
                    self.log("  Tarama modu KAPALI")
                self.scanning = False
            ...
            if wp.action == "hover":
                ...
                self._wp_scan_and_deliver(wp)
            elif wp.action == "descend":
                self.log(f"  Alcaliniyor ({DROP_HEIGHT:.1f})...")
                self._descend_slow(wp.x, wp.y, "waypoint", DROP_HEIGHT)
                time.sleep(1.0)
                self.log(f"  Yukseliniyor ({wp.z:.1f})...")
                move_to(wp.x, wp.y, wp.z)
                self.drone_pos = {"x": wp.x, "y": wp.y, "z": wp.z}
            elif wp.action == "speed_slow":
                self.fly_speed = SPIRAL_FLY_SPEED * SPEED_SLOW_MULT
                self.log(f"  Hiz: YAVAS (x{SPEED_SLOW_MULT:.1f})")
            elif wp.action == "speed_fast":
                self.fly_speed = SPIRAL_FLY_SPEED * SPEED_FAST_MULT
                self.log(f"  Hiz: HIZLI (x{SPEED_FAST_MULT:.1f})")
            elif wp.action == "log":
                self.log(f"  ISARET: konum {i+1} ({wp.x:.1f},{wp.y:.1f})")
        self.wp_idx = total
...
def draw_footer(screen, win_w, win_h, detections, stats, mission_log,
                mission_running, payloads, active_mode, color_order, log_scroll=0):
    ...
    dy += 10
    total_log = len(mission_log)
    log_top = dy + 16   # etiket satirindan sonra
    avail_h = max(0, (win_h - 100) - log_top)   # alt tus ipucu bloguna kadar olan alan
    LOG_VISIBLE = max(6, avail_h // 15)
    max_scroll = max(0, total_log - LOG_VISIBLE)
    scroll = max(0, min(log_scroll, max_scroll))
    if scroll == 0:
        log_slice = mission_log[-LOG_VISIBLE:] if total_log else []
    else:
        end_idx = total_log - scroll
        start_idx = max(0, end_idx - LOG_VISIBLE)
        log_slice = mission_log[start_idx:end_idx]
    label = "LOG:" if scroll == 0 else f"LOG (gecmis -{scroll}, [End]=alta don):"
    draw_text(screen, label, (px, dy), 10, C_DIM)
    dy += 16
...
class LiveViewer:
    def __init__(self):
        ...
        self.path_mode      = "line"      # "line" | "curve" — waypoint'ler arasi rota sekli

        # Ev (kalkis/donus) konumu — planlama ekraninda orta-tik ile tasinir, [0] ile sifirlanir
        self.home_x = HOME_X_DEFAULT
        self.home_y = HOME_Y_DEFAULT

        # Harita gorunumu (H): mission calisirken de acilabilir, render YOK —
        # sadece son cekilen goruntu uzerine drone konumu isaretlenir
        self.show_map_overlay = False

        # Waypoint surukleme (konumunu tasima) icin gecici state
        self.dragging_wp       = None
        self.drag_orig_xy      = None
        ...
        self.mission_log = ["1=Hizli  2=Normal  3=Kesin  T=Sira  A=Ayar  G=Baslat  P=Plan  H=Harita  B=RotaSekli"]
        self.log_scroll  = 0   # 0 = en alt (guncel), tekerlek/PgUp-PgDn ile gecmise gidilir
    ...
    def _route_segments(self, rect):
        """Waypoint'ler arasi rota parcalarini (renk, ekran-noktalari) olarak
        dondurur. Her parcanin rengi, o parcayi BASLATAN waypoint'in
        aksiyonuna gore belirlenir (mesela speed_fast konuldugu waypoint'ten
        bir sonrakine kadarki yol kirmizi olur). Line/curve moduna gore
        duz ya da Catmull-Rom ile yumusatilmis noktalar uretir."""
        wps = self.waypoints
        n = len(wps)
        segments = []
        use_curve = (self.path_mode == "curve" and n >= 3)
        for i in range(n - 1):
            wp0, wp1 = wps[i], wps[i+1]
            color = WP_ACTION_COLORS.get(wp0.action, C_CYAN)
            if use_curve:
                prev = wps[i-1] if i-1 >= 0 else wp0
                nxt  = wps[i+2] if i+2 < n else wp1
                pts4 = ((prev.x,prev.y), (wp0.x,wp0.y), (wp1.x,wp1.y), (nxt.x,nxt.y))
                world_pts = [(wp0.x, wp0.y)]
                for s in range(1, CURVE_SAMPLES_PER_SEGMENT + 1):
                    t = s / CURVE_SAMPLES_PER_SEGMENT
                    world_pts.append(_catmull_rom_point(*pts4, t))
            else:
                world_pts = [(wp0.x, wp0.y), (wp1.x, wp1.y)]
            screen_pts = [self._plan_world_to_screen(wx, wy, rect) for wx, wy in world_pts]
            segments.append((color, screen_pts))
        return segments
    ...
    def _draw_planning_canvas(self, rect):
        ...
        # Ev (taşınabilir kalkış/dönüş noktası)
        ox, oy = self._plan_world_to_screen(self.home_x, self.home_y, rect)
        ...
        # Waypoint'ler ve aralarındaki rota çizgisi (duz veya egri) —
        # her parca, onu baslatan waypoint'in aksiyon rengiyle cizilir
        pts = [self._plan_world_to_screen(wp.x, wp.y, rect) for wp in self.waypoints]
        if pts:
            pygame.draw.line(screen, C_GREEN, (ox, oy), pts[0], 2)
        for color, screen_pts in self._route_segments(rect):
            if len(screen_pts) >= 2:
                pygame.draw.lines(screen, color, False, screen_pts, 3)
        ...
        # Ust bilgi çubuğu
        info = (f"Tikla: konum ekle | Konuma tikla: aksiyon degistir | Konumu surukle: tasi | "
                f"Konuma sag-tik: sil | Cizgiye sag-tik: araya ekle | Bos yere sag-tik: sonuncuyu sil | "
                f"Orta tik: ev tasi | [0]Ev sifirla | "
                f"[K]Aksiyon:{WP_ACTION_LABELS.get(self.wp_action_cursor,'?')} | "
                f"[M]Rota:{'Manuel' if self.mission_type=='waypoints' else 'Spiral'} | "
                f"[B]Sekil:{'Egri' if self.path_mode=='curve' else 'Duz'}")
        info2 = "[+/-]Zoom | [Yon]Kaydir | [V]Yenile | [C]Temizle | [P]Cik"
        draw_text(screen, info, (rect.x + 10, rect.y + 8), 11, C_CYAN)
        draw_text(screen, info2, (rect.x + 10, rect.y + 24), 11, C_CYAN)
        draw_text(screen, f"Konum sayisi: {len(self.waypoints)}   "
                          f"Zoom: x{self.map_scale_mult:.2f}   "
                          f"Pan: ({self.map_offset_x:.1f}, {self.map_offset_y:.1f})",
                  (rect.x + 10, rect.y + 40), 11, C_TEXT)
    ...
    # ── Harita gorunumu (H): drone ucarken de acilabilir, RENDER YOK ──
    # Son cekilen harita goruntusu uzerine sadece drone/ev/rota isaretlenir.
    def _draw_map_overlay(self, rect):
        screen = self.screen
        ...
        if self.map_bounds is not None:
            # Ev
            hx, hy = self._plan_world_to_screen(self.home_x, self.home_y, rect)
            pygame.draw.circle(screen, C_GREEN, (hx, hy), 6, 2)
            draw_text(screen, "EV", (hx + 8, hy - 8), 11, C_GREEN)

            # Planlanmis rota (varsa) — her parca kendi aksiyon rengiyle
            pts = [self._plan_world_to_screen(wp.x, wp.y, rect) for wp in self.waypoints]
            if pts:
                pygame.draw.line(screen, C_GREEN, (hx, hy), pts[0], 2)
            for color, screen_pts in self._route_segments(rect):
                if len(screen_pts) >= 2:
                    pygame.draw.lines(screen, color, False, screen_pts, 3)
            m = self._mission
            active_idx = m.wp_idx if (m.running and m.mission_type == "waypoints") else -1
            for i, (sx, sy) in enumerate(pts):
                col = C_ORANGE if i == active_idx else C_DIM
                pygame.draw.circle(screen, col, (sx, sy), 5, 1)
            ...
            hint = pil_render_text("Harita henuz cekilmedi — [R] ile cek", 14, C_DIM)
            screen.blit(hint, hint.get_rect(center=(rect.x + rect.w//2, rect.y + rect.h//2)))
        ...
        draw_text(screen, "[H] Haritayi kapat | [R] Haritayi yenile (tek seferlik cekim, otomatik yenilenmez)",
                  (rect.x + 10, rect.y + 8), 11, C_CYAN)
    ...
    def _render(self, click_for_settings=None):
        ...
        draw_footer(self.screen, self.win_w, self.win_h, self.detections, self.stats,
                    ...
                    self.active_mode, self.color_order, self.log_scroll)

        if self.planning_mode:
            self._draw_planning_canvas(self._cam_rect())
        elif self.show_map_overlay:
            self._draw_map_overlay(self._cam_rect())
        elif self.panels:
        ...
        while not self.should_quit:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if btn_rect.collidepoint(pos_click):
                        ...
                        if cam_rect.collidepoint(pos_click):
                            ...
                            for wp in self.waypoints:
                                sx, sy = self._plan_world_to_screen(wp.x, wp.y, cam_rect)
                                if (sx-pos_click[0])**2 + (sy-pos_click[1])**2 <= 12*12:
                                    hit_wp = wp
                                    break
                            if hit_wp is not None:
                                self.dragging_wp       = hit_wp
                                self.drag_orig_xy      = (hit_wp.x, hit_wp.y)
                                self.drag_start_screen = pos_click
                            else:
                    ...
                    if self.planning_mode and not self._mission.running:
                        cam_rect = self._cam_rect()
                        if cam_rect.collidepoint(event.pos):
                            px_, py_ = event.pos
                            hit_wp = None
                            for wp in self.waypoints:
                                sx, sy = self._plan_world_to_screen(wp.x, wp.y, cam_rect)
                                if (sx-px_)**2 + (sy-py_)**2 <= 12*12:
                                    hit_wp = wp
                                    break
                            if hit_wp is not None:
                                idx = self.waypoints.index(hit_wp)
                                self.waypoints.remove(hit_wp)
                                self._add_log(f"Konum {idx+1} silindi ({len(self.waypoints)} kaldi)")
                            else:
                                seg_hit = None
                                for i in range(len(self.waypoints) - 1):
                                    wp0, wp1 = self.waypoints[i], self.waypoints[i+1]
                                    sx0, sy0 = self._plan_world_to_screen(wp0.x, wp0.y, cam_rect)
                                    sx1, sy1 = self._plan_world_to_screen(wp1.x, wp1.y, cam_rect)
                                    if self._dist_point_to_segment(px_, py_, sx0, sy0, sx1, sy1) <= 8:
                                        seg_hit = i
                                        break
                                if seg_hit is not None:
                                    wx, wy = self._plan_screen_to_world(px_, py_, cam_rect)
                                    self.waypoints.insert(
                                        seg_hit + 1, Waypoint(wx, wy, DRONE_HEIGHT, self.wp_action_cursor))
                                    self._add_log(
                                        f"Konum {seg_hit+2} araya eklendi ({wx:.1f},{wy:.1f}) — "
                                        f"sonrakiler kaydi")
                                elif self.waypoints:
                                    self.waypoints.pop()
                    ...
                    if self.planning_mode and not self._mission.running:
                        ...
                        if cam_rect.collidepoint(event.pos):
                            wx, wy = self._plan_screen_to_world(
                                event.pos[0], event.pos[1], cam_rect)
                            self.home_x, self.home_y = wx, wy
                            self._add_log(f"Ev konumu tasindi ({wx:.1f},{wy:.1f})")

                elif event.type == pygame.MOUSEMOTION:
                    if self.dragging_wp is not None and self.planning_mode:
                        cam_rect = self._cam_rect()
                        wx, wy = self._plan_screen_to_world(
                            event.pos[0], event.pos[1], cam_rect)
                        self.dragging_wp.x, self.dragging_wp.y = wx, wy

                elif event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                    if self.dragging_wp is not None:
                        moved = math.hypot(event.pos[0]-self.drag_start_screen[0],
                                          event.pos[1]-self.drag_start_screen[1])
                        if moved <= 5:
                            # Gercek bir surukleme olmadi -> tek tik, aksiyonu degistir
                            self.dragging_wp.x, self.dragging_wp.y = self.drag_orig_xy
                            idx = WP_ACTIONS.index(self.dragging_wp.action)
                            self.dragging_wp.action = WP_ACTIONS[(idx+1) % len(WP_ACTIONS)]
                            self._add_log(f"Konum aksiyonu -> {WP_ACTION_LABELS[self.dragging_wp.action]}")
                        else:
                            self._add_log(
                                f"Konum tasindi ({self.dragging_wp.x:.1f},{self.dragging_wp.y:.1f})")
                        self.dragging_wp = None

                elif event.type == pygame.MOUSEWHEEL:
                    # sag panel (LOG) uzerinde tekerlek: gecmise git/geri don
                    ...
                    elif k == pygame.K_b and not self._mission.running:
                        self.path_mode = "curve" if self.path_mode == "line" else "line"
                        self._add_log(f"Rota sekli: {'Egri' if self.path_mode=='curve' else 'Duz'}")
                    elif k == pygame.K_0 and self.planning_mode and not self._mission.running:
                        self.home_x, self.home_y = HOME_X_DEFAULT, HOME_Y_DEFAULT
                        self._add_log(f"Ev konumu sifirlandi ({HOME_X_DEFAULT:.1f},{HOME_Y_DEFAULT:.1f})")
                    elif k == pygame.K_h and not self.planning_mode:
                        self.show_map_overlay = not self.show_map_overlay
                        if self.show_map_overlay:
                            self._add_log("Harita gorunumu ACIK")
                            if self.plan_bg_bgr is None and not self.plan_bg_capturing:
                                if self.map_entered:
                                    self._start_map_recapture()
                                else:
                                    self._start_map_enter()
                        else:
                            self._add_log("Harita gorunumu KAPALI")
                    elif k == pygame.K_r and self.show_map_overlay and not self.plan_bg_capturing:
                        if self.map_entered:
                            self._start_map_recapture()
                        else:
                            self._start_map_enter()
                        self._add_log("Harita yenileniyor...")
